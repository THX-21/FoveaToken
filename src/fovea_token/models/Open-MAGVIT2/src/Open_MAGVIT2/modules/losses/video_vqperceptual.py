...
from ..discriminator.model import NLayerDiscriminator3D, weights_init
...
def non_saturate_gen_loss(logits_fake):
    """
    logits_fake: [B 1 T H W]: 3D
    logits_fake: [B*T 1 H W]: 2D 
    """
    B, _, _, _, _ = logits_fake.shape
...
class VQLPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start, codebook_weight=1.0, pixelloss_weight=1.0,
                 disc_num_layers=4, disc_in_channels=3, disc_factor=1.0, disc_weight=1.0,
                 ...
                 disc_ch=64, disc_in_ch=3, disc_ch_mult = [2, 4, 8, 8], disc_out_ch = 1):
        super().__init__()
        ...
        if disc_loss == "hinge":
            ...
            self.discr_loss = non_saturate_discriminator_loss
        else:
    ...
    def forward(self, codebook_loss, loss_break, inputs, reconstructions, optimizer_idx,
        ...
        """
        input is video format: [B C T H W]
        """
        rec_loss = torch.abs(flatten_t_dim(inputs).contiguous() - flatten_t_dim(reconstructions).contiguous())
        nll_loss = rec_loss.clone()
        if self.perceptual_weight > 0:
            ## the input is in the video format: [B C T H W]
            p_loss = self.perceptual_loss(flatten_t_dim(inputs).contiguous(), flatten_t_dim(reconstructions).contiguous())
        ...
        if optimizer_idx == 1:
            ...
            if cond is None:
                logits_real = self.discriminator(inputs.contiguous().detach()) #[1 1 3 16 16]

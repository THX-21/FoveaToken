...
def instantiate_from_config(config):
    ...
    return getattr(import_module(module_name), class_name)(**config.get("params", {}))
...
class Net2NetTransformer(L.LightningModule):
    def __init__(self,
        ...
        if permuter_config is None:
            permuter_config = {"target": "src.IBQ.modules.transformer.permuter.Identity"}
    ...
    def state_dict(self, *kwargs, destination=None, prefix='', keep_vars=False):
        '''
        super().state_dict()和super().state_dict(*args, destination, prefix, keep_vars)的区别主要就是prefix
        比如transformer训练，会多一个first_stage_model.
        奇怪的点，确实已经去掉incpetion和lpips的model了，但是依然保存下来了
        '''
    ...
    def forward(self, x, c):
        ...
        if self.token_factorization:
            ...
            cz_indices = (a_indices_pre[:, :-1], a_indices_post[:, :-1], c_indices)
            ...
            logits_pre = logits_pre[:, c_indices.shape[1]-1:]
            logits_post = logits_post[:, c_indices.shape[1]-1:]
    ...
    def encode_to_z(self, x):
        ...
        if isinstance(indices, tuple):
            ...
            # indices_pre = self.permuter(indices_pre)
            # indices_post = self.permuter(indices_post)
    ...
    def decode_to_img(self, index, zshape):
        if self.token_factorization: #using token factorization
            ...
            bhwc = (zshape[0], zshape[2], zshape[3], zshape[1] // 2)

...
class Qwen3_5ForConditionalGeneration(Qwen3_5PreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Overwritten because visual inputs are only needed on the first decoding
        # step. After the multimodal prefix has been encoded into cache, passing
        # the raw images/videos again would be redundant and expensive.

        # The parent helper handles normal text-generation bookkeeping such as
        # slicing to the last token when cache is present.
    ...
    def _prepare_position_ids_for_generation(self, inputs_tensor, model_kwargs):
        # Overwritten because multimodal generation needs four logical position
        # streams: one text stream plus three MRoPE streams (temporal, height,
        # width).
        ...
        # Otherwise compute 3D position ids for any multimodal prefix and
        # concatenate them with the normal text position ids expected by the
        # generation helpers.
        ...
        if (
            ...
            # Fall back to text-like positions for all 3 visual streams.
    ...
    def _get_image_nums_and_video_nums(
        ...
        # This helper is only used during generation expansion where packed
        # visual tensors lack a batch dimension. We recover how many images and
        # videos belong to each sample by inspecting placeholder-token patterns.
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        #
        # Beam expansion is straightforward for batched tensors shaped `(bs, ...)`
        # but visual tensors here are packed across the whole batch. This helper
        # first reconstructs per-sample slices, repeats them, and then packs them
        # back together.
        ...
        def _expand_dict_for_generation_visual(dict_to_expand):
            ...
            def _repeat_interleave_samples(x, lengths, repeat_times):
                # Repeat packed per-sample slices while preserving the original
                # within-sample token order.
                # x: (sum(sample_lengths), ...) where dim 0 packs all samples.
                # lengths: len == batch_size, with one packed length per sample.
                samples = torch.split(x, lengths)
                # samples: tuple[(sample_i_length, ...)].
                repeat_args = [repeat_times] + [1] * (x.dim() - 1)
                # Each sample_i: (sample_i_length, ...) -> (repeat_times * sample_i_length, ...),
                # then concatenated back to packed form.
                result = torch.cat([sample.repeat(*repeat_args) for sample in samples], dim=0)
                # result: (sum(sample_lengths) * repeat_times, ...)
                return result
        ...
        # First repeat packed visual tensors sample-by-sample, then repeat
        # ordinary batched tensors along batch dimension.
